...
- **RQ4.** What accuracy-versus-latency trade-off does the signal-processing pipeline (filtering, windowing, feature extraction) impose, and which configuration best balances the two?
  - *Evidence to date:* the chosen configuration — 250 ms (64-sample) RMS window, 2nd-order Butterworth bands, EMA `α = 0.3` (~150 ms), and a 5-tick (250 ms) forward debounce — was selected as a time-frequency compromise: long enough to estimate band power stably, short enough to keep detection delay ~125 ms median. The qualitative trade-off is documented (larger windows / higher-order filters → sharper bands but more group delay); a formal parameter sweep quantifying accuracy vs latency across settings is still to be run.
- **RQ5.** To what extent does per-session, noise-floor-based adaptive calibration improve robustness against signal drift relative to fixed thresholds, and does the user-tuned sensitivity parameter transfer across sessions?
  - *Evidence to date:* fixed μV thresholds were observed to misfire across days and within minutes (headset fit, skin chemistry), motivating the move to `θ = σ·P₉₀(baseline)` with a 5 s noise-floor capture. Qualitatively, the **unitless sensitivity `σ` transfers** across sessions (defaults forward 1.2×, reverse 3×, held over multiple sessions) while the absolute threshold is re-derived each time. A quantitative drift study over a 30-minute session (Chapter 4.2) is still pending.
...
- **RQ7.** What are the dominant sources of false positives and false negatives per gesture, and how can they be mitigated?
  - *Evidence to date:* identified FP sources — forward: speech, swallowing, neck tension; reverse: forehead expressions, surprise reactions. Mitigations in place: the ratio discriminator (anti cross-fire), a ZCR > 0.20 gate (rejects slow drift), forward debounce, and a held-reverse model with a trigger debounce. Validation recorded **0 false positives over 29 s of rest**; however that rest was quiet, so the speech/swallow FP rates are characterised qualitatively and not yet quantified under active interference.
...
```
Forward (jaw clench):  TP > θ_TP  AND  r < r_fwd  AND  max_c ZCR_c > 0.20   [5-tick debounce]
Reverse (eyebrow):     AF > θ_AF  AND  r > r_rev  AND  max_c ZCR_c > 0.20   [held while raised]
```

with a **dead band** `r_fwd = 0.6`, `r_rev = 1.0` (the 0.6–1.0 gap is a no-trigger zone absorbing cross-session ratio drift; empirical: measured clench `r ∈ [0.2, 0.6]`, eyebrow `r ∈ [1.5, 1.9]`). Forward suppresses reverse on co-trigger. Steering is taken from the accelerometer Y-axis with a dead zone of ±0.1.
...
**HEA-18 (steering) finalized.** The live steering tuned on 2026-06-15 was confirmed on the moving chassis and its sensitivity locked: `DEFAULT_STEER_GAIN = 4.0` — the value giving a proportional tilt→turn feel (a higher gain saturated the steer command at a small tilt, making it feel binary rather than proportional), with firmware turn sharpness fixed at `STEER_DIFFERENTIAL = 1.0` (sharp pivot; no brown-out observed at this level). A slight left veer when driving straight remains, deferred to a polish pass (a small `MOTOR_TRIM` correction).

**HEA-19 (drive-watchdog / emergency stop) verified — the safety basis for live demonstration.** The 500 ms drive-watchdog (`WATCHDOG_MS`: motors cut if no `drive:` command arrives within the window) had never been deliberately exercised end-to-end. It was tested under three realistic mid-drive disconnect modes, driving the car with a *sustained* jaw clench (wheels elevated for safety) so commands flowed right up to the moment of the cut — meaning only the watchdog could account for a stop. In all three the wheels stopped **effectively instantly**, well within the 500 ms budget:

1. **Network loss** — phone hotspot switched off (car *and* laptop lose the network).
2. **Client-app close** — dashboard browser tab closed (graceful WebSocket close).
3. **Commander drop** — laptop WiFi switched off while the hotspot and the car stayed connected (ungraceful loss of the controlling client only).

Mode 3 is the decisive case: the car itself remained on the network and its WebSocket server saw no clean disconnect, yet it still stopped — confirming the watchdog is purely *time-based* (no drive message → stop) and does not rely on the ESP32 detecting that anything is wrong. This is the realistic "laptop slept / crashed / walked out of range" failure, and exactly the case where a naive implementation would keep driving on the last command.

**Methodological note.** The original acceptance listed "kill the dev server" as the third mode; it was replaced with "laptop WiFi off." Because the browser talks **directly** to the ESP32 over WebSocket, once the dashboard page is loaded, killing the Next.js dev server does not interrupt the command path (the already-loaded tab keeps sending) — so it would not exercise the watchdog at all. The substituted mode is both a genuine disconnect and the more demanding, ungraceful one.

**Explicit-stop decision (acceptance item).** Whether to add a UI kill control is answered *yes, implemented*: an arm/"listening" toggle gates all Muse-driven control while leaving the headset connected and the manual controls live, and defaults to *disarmed* so the car never moves on connect. A button was chosen over a stop *gesture* — using a gesture to disable gesture control is self-defeating (a noisy artifact could toggle it). The time-based watchdog (last line of defence) and the explicit arm toggle (operator control) thus give two independent ways to stop the vehicle.

This verification also exercises the motor-stop path of the `applyMotors()` rewrite (signed per-side mixing / counter-rotation) done just prior — the stop path is sound under the new code.

**HEA-21 (gesture thresholds) validated and closed.** With today's headset fit (all four contacts green), the forward/reverse discrimination tuned on 2026-06-15 was confirmed on the live car: a clench drives forward *only* and an eyebrow raise reverses *only*, with no cross-firing, and — critically — the meters sit at 0 and the car holds still through normal talking and head movement (no resting false fires). This session's AF/TP ratios again fell cleanly outside the 0.6–1.0 dead band, so the dead-band approach (rather than a single cutoff) transferred across the re-fit as intended (**RQ5**). One deliberate design choice recorded: the forward trigger is left as a live dashboard slider rather than a fixed constant, because this user's *comfortable*-clench-to-rest amplitude gap is narrow (~1.2×), making the effort-vs-false-fire balance best tuned per fit — with the ¼-second debounce doing the real work of separating a *held* clench from brief resting noise. Quantitative hit-rate / false-positive figures for Chapter 4 will come from the HEA-16 recorded run.

### 2026-06-15 (extended) — First live closed-loop drive, and the Sprint-2 tuning it forced (HEA-10 done; HEA-18/HEA-21 tuning; HEA-19 stop control)

With the L298N driver working and all four chassis motors driven (earlier entry today), the first sustained **Muse → car closed-loop** session was run: a jaw clench detected in the browser physically drove the vehicle forward. This closes HEA-10 (the gateway — *the car reacts to the Muse*) and is the first direct evidence for **RQ1** (live closed-loop drive), which every prior validation had only approached via bench replays. Driving the real car immediately surfaced a cluster of problems the bench/replay rig could not, and the session became a tuning pass. All software below is code-complete and lint-clean; items needing bench re-confirmation are flagged.

**Summary of parameter changes (for Chapter-4 reproducibility):**

| Parameter (file) | Before | After | Reason |
|---|---|---|---|
| forward sensitivity default (`page.tsx`) | ×2.0 | ×1.2 + live slider (floor 1.5→1.0) | comfortable clench = only 62 % of the old threshold |
| forward debounce | 150 ms (3 ticks) | 250 ms (5 ticks) | reject brief noise spikes near the lowered threshold |
| `INTENSITY_SATURATION` | 3.0 | 2.0 | full speed was unreachable (~6× baseline TP) |
| ratio discriminator | single cutoff 0.8 | dead band: forward < 0.6, reverse > 1.0 | eyebrow ratio drifted to ~1.2 this session |
| reverse actuation | 800 ms pulse + 1500 ms cooldown | held (250 ms grace) + 150 ms debounce | sustained raise produced an on-off burst |
| steering input | raw accel sent as steer | gain (single slider) → steer, clamped ±1 | weak/slow turning |
| `STEER_DIFFERENTIAL` (`car.ino`) | 0.35, shared-direction mix | 1.2, signed per-side mix (counter-rotation) | gentle radius; now sharp pivot/drift |

**1. The forward trigger sits in the noise — an SNR limit of a *comfortable* clench.** Live readouts showed a comfortable clench raises TP high-band to only ~1.24× the calibrated resting baseline (dashboard read 62 % of the old ×2 threshold; resting noise later measured ~40 % of the new ×1.2 threshold). At ×2 the trigger was unreachable without a hard clench; at ×1.2 it triggered but sat barely above resting noise, so forward self-fired. Mitigations: raised the sustained-signal debounce 150→250 ms and exposed forward sensitivity as a live slider for per-fit tuning. The underlying finding — that this user's *comfortable* masseter clench is only ~1.2× resting on TP — is a concrete consumer-EEG SNR limit and a direct instance of the accuracy-vs-effort trade-off (**RQ4**): reliable separation needs a *deliberate*, not comfortable, clench.

**2. Proportional speed scaling was unreachable.** `INTENSITY_SATURATION = 3.0` required TP at 3× threshold (~6× baseline) for full speed, so the intensity→PWM mapping (**H2**) was effectively flat in use. Lowered to 2.0 so the drive range spans a producible clench range. H2 quantification pending.

**3. The ratio discriminator drifted across sessions — single cutoff replaced by a dead band.** A deliberately *unclenched* eyebrow raise triggered *forward*. Root cause: session-to-session drift of the AF/TP ratio itself — this session's eyebrow landed at ~1.2 (vs the 1.5–1.9 measured 2026-05-29), falling below the single 1.3 cutoff into the clench band. Rather than re-chase the cutoff, the single boundary became a **dead band** — forward requires ratio < 0.6, reverse > 1.0, with 0.6–1.0 a no-trigger zone (this session's clench 0.43 and eyebrow 1.2 sit cleanly outside it). Three changes hardened it: gates now read the extra-smoothed ratio (no flicker), reverse gained a 150 ms debounce, and mutual exclusion was made symmetric (the live gesture locks out the other). This is a direct hit on **RQ5 / cross-session transferability**: the 2026-05-29 cutoff was correct for *that* session's gap but the gap moved on re-fit; a margin-bearing dead band plus per-session ratio calibration is the robust generalization.

**4. A misleading meter, not a misbehaving car.** During an eyebrow raise the Forward card pegged at 150 %. The car was *not* driving forward (the ratio gate + symmetric exclusion suppress it); the card was showing raw TP level, which this user's eyebrow raise drives high via the same temporal/auricularis co-activation that depresses the eyebrow ratio. The Forward/Reverse cards were gated by eligibility — the Forward bar reads 0 unless the ratio is in the clench zone (and reverse inactive), and vice-versa — so one gesture no longer lights the opposing indicator. Lesson: an interface read as ground truth in operation must reflect the *decision*, not an intermediate signal.

**5. Reverse de-bursted into a held gesture.** Reverse was an 800 ms pulse + 1500 ms cooldown, so a sustained eyebrow raise burst on-off-on. Reworked to a held model (each satisfied tick extends a 250 ms grace window), matching the forward "hold-while-clenching" semantics.

**6. Steering: amplify, then sharpen by counter-rotation, then collapse to one knob.** (a) The raw accelerometer roll delta is small, giving a weak steer command; added a tilt→steer gain (clamped ±1) — responsive but not sharp. (b) Turn sharpness is bounded in firmware: the original differential (0.35) only slowed the inside wheels to 65 %, so `applyMotors()` was rewritten from a shared-direction, magnitude-only mix to a **signed per-side mix** (each side = drive ± steering, direction from the sign), letting the inside wheel cross zero and counter-rotate for a pivot/drift turn. (c) After briefly exposing gain and differential as two controls, they were consolidated — on the principle that the operator wants *one* intuitive control — into a single "Steering sensitivity" slider (the gain), with the differential fixed in firmware at 1.2. Counter-rotation/sharpness verification on the moving chassis is pending a reflash.

**7. Operability and safety controls.** Added (i) an arm/"listening" toggle that gates all Muse-driven control while leaving the headset connected and manual controls live — implemented as a *button, not a gesture* (a gesture to disable gestures is self-defeating) and defaulting to *disarmed* so the car never lurches on connect; this also serves as the explicit dashboard STOP that the safety scope (**HEA-19**) asked for. (ii) Explicit Muse and car disconnect controls. (iii) Manual Forward/Reverse buttons that hold ~2.5 s (resending ahead of the 500 ms watchdog) so a wiring/direction check produces visible motion.

**Status and pending.** HEA-10 closed (brain-control chain drives the vehicle — first RQ1 evidence). HEA-18 (steering) and HEA-21 (gesture thresholds) advanced, not closed: the steering/counter-rotation change needs an ESP32 reflash and the firmware differential (1.2) is unverified on the moving chassis; the forward threshold/debounce and the dead-band separation are code-complete and the separation was confirmed qualitatively in-session, but the forward false-trigger rate and H2 speed-scaling still need quantifying from a fresh recording (none captured this session — a gap to close). The **HEA-19 watchdog three-failure-mode test** (WiFi loss, tab close, dev-server kill) remains outstanding and is now more important because `applyMotors()` was rewritten. Documentation drift introduced today (reverse is held not pulsed; new sensitivity defaults; the steering model) is tracked under **HEA-22** for README/CLAUDE reconciliation.

**Thesis-relevant lessons.** (a) Bench/replay validation did not surface the live-driving problems — the comfortable-clench SNR, the cross-session ratio drift, the meter-vs-decision confusion, the need for arm/disarm — which argues for the live closed-loop run (RQ1) as a distinct validation stage, not a formality. (b) The TP-dominant electrode physics struck a third time: it set the clench ratio low (~0.4), the eyebrow ratio modest (~1.2–1.9), and now drove the forward-amplitude SNR limit — the controls keep having to be derived from this user's measured signal, not electrode-placement theory. (c) Parameters fit to one session (the 1.3 cutoff) do not necessarily transfer to the next headset fit; margins (dead bands, debounces) and per-session recalibration are the robust response — exactly the stability question **RQ5** poses.

### 2026-06-15 — Motor actuation stage solved: four defective TB6612 modules, switch to L298N (HEA-8 driver milestone)

The vehicle's motor-actuation stage works for the first time: a DC motor driven from the ESP32 through the motor driver spins under firmware control. Reaching this required abandoning the TB6612FNG driver specified throughout the earlier build in favour of an **L298N**, after an extended hardware-fault investigation.

**The fault.** After the 2026-06-08 confirmation that the wireless command path was sound, the Stage-2 motor bench test (06-11 to 06-12) failed repeatedly and *identically* across **four** separate TB6612 modules (the original, a hand-replaced unit, and both boards of a fresh sealed two-pack). In every case the driver's low-side output transistors worked — the chip could pull an output to ground — but **neither high-side ever engaged**: under a forward command (IN1 high, IN2 low, PWM high, STBY high) the "high" output sat at 0 V instead of rising toward the motor supply, and likewise for reverse on the opposite output. The motor therefore never saw a differential voltage and never turned.

**Diagnosis.** Because four modules behaved identically, a per-unit defect was initially discounted in favour of a systemic cause, and a systematic elimination was carried out measuring at the driver's own pins. Every control input was confirmed at correct logic levels (STBY 3.3 V, IN1 3.3 V, IN2 0 V, PWM ~2.6 V and, separately, a forced steady 3.3 V); motor supply present (VM 7.34 V); logic supply tested at both 3.3 V and 5 V; all ground pins continuity-checked to the common rail; outputs checked for shorts to ground and to each other (none); both drive directions exercised. Every measurable quantity was correct, yet no high-side drive appeared. A motor connected directly across the battery spun normally, isolating the fault to the driver stage.

**Conclusion and resolution.** The dead-high-side signature, reproduced across four boards while every external condition measured correct, is the documented failure mode of **counterfeit TB6612 silicon** — the internal high-side gate-drive (charge pump) is non-functional on fake parts. The modules had been sourced as the cheapest available no-name boards, the likely supply-chain explanation. Rather than gamble on another TB6612, the driver was switched to an **L298N**, a bipolar-junction-transistor H-bridge whose topology has *no* high-side charge pump and so cannot exhibit this fault. The motor drove on the first attempt.

**Engineering trade-off.** The L298N is less efficient than the MOSFET-based TB6612 and drops ~1.5–2 V across its outputs (the motor sees ~5.8 V from the 7.4 V pack), acceptable for the demonstration vehicle. Critically, its control interface is functionally identical — two direction pins plus one PWM "enable" per motor — so `car/car.ino` needed no logic change, only a relabelling of the existing pin map (PWMA→ENA, AIN1/AIN2→IN1/IN2, PWMB→ENB, BIN1/BIN2→IN3/IN4; the TB6612 STBY pin is unused). The L298N's onboard 5 V regulator self-powers its logic from the motor supply, removing the separate logic-supply wire the TB6612 required.

**Methodological note.** Two points worth recording. First, the staged-isolation strategy adopted on 2026-06-08 — proving the `dashboard → ws:// → ESP32` comms path independently, motor stage disconnected — paid off exactly as intended: when the motor test later failed, the fault was confidently localised to the driver stage rather than re-litigating the wireless or firmware layers. Second, this is a concrete instance of a recurring project theme — building on commodity/hobby-grade hardware introduces reliability hazards (here, counterfeit components) absent from research-grade equipment, and a topology-different replacement part can serve simultaneously as a fix and as a diagnostic discriminator.

**Status.** HEA-8 driver milestone reached: a single motor drives under firmware control via a temporary continuous self-test. Remaining before HEA-8 closes and RQ1 (live closed-loop drive) can be addressed: parallel-wire all four chassis motors (left → channel A / OUT1–OUT2, right → channel B / OUT3–OUT4), remove the self-test and restore normal WebSocket/dashboard control, then run a full closed-loop drive test from the dashboard.
...
- Forward: `tpMean > tpThr_fwd AND ratio < 0.6 AND any ZCR > 0.20`, with a 5-tick (250 ms) debounce. **TP-primary** — on this hardware/user, masseter EMG lands on TP9/TP10 and barely touches AF (see 2026-05-27 entry for the empirical data justifying this).
- Reverse: `afMean > afThr_rev AND ratio > 1.0 AND any ZCR > 0.20`, held while the raise is sustained (250 ms grace window, 150 ms trigger debounce); a one-shot/simulated trigger fires a fixed 800 ms pulse.
- `ratio = afMean / max(tpMean, 1)`
- Forward suppresses reverse on co-trigger (mutual exclusion is symmetric — the live gesture locks out the other)
- Discriminator rationale: the AF/TP ratio is split by a **dead band**, not a single cutoff — clench is TP-strong/AF-weak (ratio ≈ 0.2–0.6, forward requires `< 0.6`); eyebrow is AF-strong (ratio ≈ 1.5–1.9, reverse requires `> 1.0`); the 0.6–1.0 gap is a no-trigger zone that absorbs cross-session ratio drift.
...
- Differential steering: signed per-side mix (`leftCmd = drive + steer`, `rightCmd = drive − steer`) with `STEER_DIFFERENTIAL = 1.0` and `MOTOR_TRIM = 0.0`, letting the inside wheel counter-rotate for a sharp pivot turn
...
- The counterfeit TB6612 episode (first diagnosed 2026-05-22; full four-board failure and the switch to the L298N documented 2026-06-15)
